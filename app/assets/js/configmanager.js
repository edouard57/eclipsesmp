...
const dataPath = path.join(sysRoot, '.eclipse-smp-launcher')
...
/**
 * Derive the offline-mode UUID for a username, using the same algorithm
 * vanilla Minecraft uses (MD5 name-based UUID of "OfflinePlayer:<name>").
 *
 * @param {string} username The offline account's username.
 *
 * @returns {string} The derived UUID (dashed, lowercase).
 */
function craftOfflineUuid(username){
...
/**
 * Adds an offline (non-premium) account to the database to be stored. Offline
 * accounts have no real Mojang/Microsoft session; the server is expected to
 * be configured to accept them (online-mode=false, or an auth mod such as
 * EasyAuth in mixed mode).
 *
...
exports.addOfflineAuthAccount = function(username){
    ...
    const uuid = craftOfflineUuid(trimmed)
    config.selectedAccount = uuid
    config.authenticationDatabase[uuid] = {
        type: 'offline',
        accessToken: '0',
